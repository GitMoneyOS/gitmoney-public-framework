...
# POL, VSM, ICM, and IFL Stack

## Purpose

Show how the open GitMoney OS skeleton is governed by the deeper architecture stack.

The public skeleton is simple:

```text
Source Memory -> Agent Workbench -> GitHub Record
```

That skeleton is not arbitrary. It is the public operating body for a deeper method:
...
- **ICM** structures context, stages, artifacts, and handoffs so humans and agents can execute bounded work.
- **IFL** investigates the evidence, names the operating pathology, and designs the intervention.
- **GitMoney OS** gives the evidence a public skeleton and a court-of-record pattern.

The skeleton is open. The living machine is paid.

## The public skeleton

GitMoney OS uses three public operating layers:
...
The GitHub Record is where approved work becomes inspectable.
...
How does it learn from its own work?
What must be preserved, revised, retired, or tested next?
```

POL turns scattered material into a living commitment loop.
...
```text
What functions must exist for this business to remain viable?
Who owns operations, coordination, control, intelligence, and policy?
Where does the system fail to regulate itself?
```

VSM turns a business into an inspectable operating system.
...
```text
What context does this bounded job need?
...
ICM turns work into bounded execution.

Inside the public skeleton:

- ICM structures Agent Workbench activity.
- ICM defines contracts for context, stage, artifact, and handoff.
- ICM lets humans and agents share a visible operating surface instead of relying on vibes, chat memory, or prompt archaeology.
...
```text
Where is the identity of the business expressed in behavior?
Where does the evidence contradict the claim?
Where is the founder still middleware?
Where does AI amplify dysfunction?
What intervention does the record support?
```

IFL turns evidence into diagnosis and intervention.

Inside the public skeleton:

- IFL explains why the proof package matters.
...
| Architecture layer | Public function | Private function | Paid implication |
|---|---|---|---|
| POL | Names identity, commitments, and learning loop | Interprets founder and company doctrine | Audit discovers broken commitments |
| VSM | Maps viability functions, owners, and control loops | Diagnoses control and coordination failures | Audit names system pathology |
...
The public architecture stack must not expose:
...
- pricing floor math beyond the approved public ladder;
- skill source bytes;
