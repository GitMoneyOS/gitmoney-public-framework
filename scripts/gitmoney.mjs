...
function runInit(args) {
  ...
  const starterSource = path.join(REPO_ROOT, "starter");
  if (!existsSync(starterSource)) {
    console.error(`[Error] Starter source not found at: ${starterSource}`);
    process.exit(1);
  }
...
`;
  ...
  if (withVault) {
    const vaultSource = path.join(REPO_ROOT, "templates", "starter-vault");
    ...
    for (const f of vaultMdFiles) {
      ...
      // Point skill link to local project .agents/skills based on nesting depth
      ...
      // Point starter folder link to parent project root
      content = content.replace(/\.\.\/\.\.\/starter\//g, relToTarget ? relToTarget + "/" : "./");
      // Point root AGENTS.md link to parent project AGENTS.md
      content = content.replace(/\[\.\.\/AGENTS\.md\]\(\.\.\/AGENTS\.md\)/g, "[AGENTS.md](" + (relToTarget ? relToTarget + "/" : "./") + "AGENTS.md)");
      // Point framework templates to canonical GitHub repo URLs
  ...
  console.log("\n[GitMoney Init] Complete. Next steps:");
  ...
  console.log("  4. Run `node scripts/gitmoney.mjs doctor` to verify health.");
}

// ----------------------------------------------------
// COMMAND: doctor
// ----------------------------------------------------
function runDoctor(targetDir = process.cwd()) {
  console.log("==================================================");
  ...
  const coreFiles = isFramework
    ? ["AGENTS.md", "README.md", "START-HERE.md", "SECURITY.md", "SPEC_VERSION", "gitmoney.yaml"]
    : ["AGENTS.md", "00_COCKPIT.md", "gitmoney.yaml"];
  ...
  if (existsSync(operatorPath)) {
    ...
    if (opContent.includes("gitmoney-public-operator") && opContent.length > 200) {
      console.log(`  [OK] gitmoney-public-operator skill verified (${path.relative(targetDir, operatorPath)}).`);
    } else {
  ...
  console.log("\n--- 3. Manifest Validity (gitmoney.yaml) ---");
  const manifestPath = path.join(targetDir, "gitmoney.yaml");
  if (existsSync(manifestPath)) {
    const content = readFileSync(manifestPath, "utf8");
    const requiredKeys = ["spec_version", "installed_spec_version", "owner", "profile", "operator_version", "schema_version", "required_artifacts"];
    let mFailed = false;
    for (const k of requiredKeys) {
      if (!new RegExp(`^${k}:`, "m").test(content)) {
        console.error(`  [FAIL] gitmoney.yaml missing property: ${k}`);
        failures++;
        mFailed = true;
      }
    }
    if (!mFailed) {
      console.log("  [OK] gitmoney.yaml conforms to required manifest properties.");
    }
  } else {
  ...
  // 5. Private Source Exposure Scan
  console.log("\n--- 5. Private Source Exposure Scan ---");
...
if (!command || command === "--help" || command === "-h") {
  ...
  console.log("  init <dir> [--with-vault]   Initialize a new AI Office in <dir>");
  console.log("  init --here [--with-vault]  Initialize an AI Office in current directory");
  console.log("  doctor                      Verify repository architecture, boundaries, and health");
  process.exit(0);
}
